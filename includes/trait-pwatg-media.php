...
trait PWATG_Media_Trait {
	...
	public function add_media_modal_action_field( $form_fields, $post ) {
		...
		$form_fields['pwatg_generate_alt'] = [
			'label' => __( 'Alt Text Generator', 'presswell-alt-text' ),
			'input' => 'html',
			...
			'helps' => __( 'Runs AI alt text generation for this image and reloads with a status notice.', 'presswell-alt-text' ),
		];
	...
	public function handle_single_generation() {
		...
		check_admin_referer( 'pwatg_generate_single_' . $attachment_id );
	...
	public function get_last_generated_label( $attachment_id ) {
		$raw = (string) get_post_meta( $attachment_id, '_pwatg_last_generated', true );

		if ( '' === trim( $raw ) ) {
			return __( 'Never', 'presswell-alt-text' );
		}
		...
		if ( false === $timestamp || $timestamp <= 0 ) {
			return __( 'Unknown', 'presswell-alt-text' );
		}

		return sprintf(
			/* translators: %s: localized datetime */
			__( '%s', 'presswell-alt-text' ),
	...
	public function render_admin_notices() {
		...
		if ( isset( $_GET['pwatg_single'] ) ) {
			...
			$messages = [
				'updated'     => __( 'Alt text generated successfully.', 'presswell-alt-text' ),
				'skipped'     => __( 'No changes were needed for this image.', 'presswell-alt-text' ),
				'missing_key' => __( 'Missing API key. Add it in Alt Text Generator settings.', 'presswell-alt-text' ),
				'error'       => __( 'Could not generate alt text for this image.', 'presswell-alt-text' ),
			];
			...
			if ( isset( $messages[ $status ] ) ) {
				...
				if ( $attachment > 0 ) {
					$text .= ' ' . sprintf(
						...
						__( 'Attachment ID: %d.', 'presswell-alt-text' ),
						$attachment
					);
				}
		...
		if ( ! empty( $_GET['page'] ) && 'presswell-alt-text-generator' === $_GET['page'] ) {
			$test_notice = get_transient( 'pwatg_test_notice' );
			if ( is_array( $test_notice ) && isset( $test_notice['message'] ) ) {
				delete_transient( 'pwatg_test_notice' );
		...
		if ( empty( $_GET['page'] ) || 'presswell-alt-text-bulk-generator' !== $_GET['page'] ) {
			return;
		}

		$notice = get_transient( 'pwatg_bulk_notice' );
		if ( ! is_array( $notice ) ) {
			return;
		}

		delete_transient( 'pwatg_bulk_notice' );

		$message = sprintf(
			/* translators: 1: processed count, 2: updated count, 3: failed count */
			esc_html__( 'Bulk generation complete. Processed: %1$d, Updated: %2$d, Failed: %3$d.', 'presswell-alt-text' ),
