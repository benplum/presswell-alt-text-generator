jQuery(function($) {
  ...
  const fieldName = data.fieldName || 'pwatg_generate_alt';
  const compatFieldSelector = '.compat-field-' + fieldName;
  const compatRowSelector = 'tr' + compatFieldSelector;
  ...
  function setLastGeneratedForButton($button, lastGenerated) {
    ...
    if (!target.length) {
      const fieldScope = $button.closest('.setting, td, ' + compatFieldSelector);
